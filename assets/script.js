...
const noMessages = [
  "Ơ, suy nghĩ lại đi mà 🥺",
  "Bấm nhầm rồi đúng hông?",
];
let noClickCount = 0;
...
noBtn.addEventListener("click", () => {
  ...
  if (noClickCount >= 3) {
    noBtn.disabled = true;
    noBtn.classList.add("taken-away");
    noPlane.classList.add("fly");
    yesBtn.style.setProperty("--scale", 1.48);
    yesBtn.classList.add("celebrate");
    questionHint.textContent = "Máy bay chở mất nút Không rồi, giờ chọn Có nha 😝";
  ...
  questionHint.textContent = noMessages[noClickCount - 1];
});

yesBtn.addEventListener("click", () => {
  noPlane.classList.remove("fly");
  noPlane.hidden = true;
  questionStep.classList.remove("active");
  questionStep.classList.add("leaving");

  window.setTimeout(() => {
    questionStep.classList.remove("leaving");
    questionStep.setAttribute("aria-hidden", "true");
...
passwordForm.addEventListener("submit", (event) => {
  ...
  const normalizedPassword = passwordInput.value.replace(/[^0-9]/g, "");

  if (normalizedPassword !== "13042006") {
    passwordError.textContent = "Chưa đúng rồi, thử nhớ lại ngày đặc biệt nha ♡";
  ...
  passwordError.textContent = "Đúng rồi! Chào mừng em đến với món quà này 💕";
  document.body.classList.add("gate-open");
  loveGate.classList.add("is-opening");
